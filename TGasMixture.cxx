...
Double_t TGasMixture::GetPhotoelectricCrossSection(Double_t ENERGY)
{
  ...
  for (Int_t i=0; i<nElements; i++){
    CrossSection += Elements[i]->GetPhotoelectricCrossSection(ENERGY)*Elements[i]->GetDensity()/Density;
  }
...
//*******************************************************************************
 std::vector<TGraph*>TGasMixture::GetAbsProbGraph(Double_t EnI){
   const Int_t nPoints = N_ENERGY_BINS;
   Double_t Energy[nPoints];
   //Double_t AbsorptionProbability[(const Int_t)nElements][nPoints];
   vector <double> AbsorptionProbEl;
   //vector <AbsorptionProbEl> AbsorptionProbability;
   gPad->SetLogx();
   gPad->SetLogy();
   ...
   for (Int_t j=0; j<nElements; j++){
     ...
     for (Int_t i=0; i<nPoints; i++){
       Energy[i] = EnI+i*0.5;
       AbsorptionProbEl.push_back(
	 (Elements[j]->GetPhotoelectricCrossSection(Energy[i])*Elements[j]->GetDensity())/
	 (GetPhotoelectricCrossSection(Energy[i])*Density));
     }
   
     TGraph* AbsorptionProbabilityGraphs = new TGraph(nPoints, Energy, &AbsorptionProbEl[0]);
     AbsorptionProbabilityGraphs->SetTitle("AbsorptionProbabilities");
     AbsorptionProbabilityGraphs->SetMarkerStyle(26);
     AbsorptionProbabilityGraphs->SetMarkerSize(0.7);
     AbsorptionProbabilityGraphs->SetMarkerColor(j+1);
     if(j==0){
       AbsorptionProbabilityGraphs->GetXaxis()->SetTitle("Photon energy (keV)");
       AbsorptionProbabilityGraphs->GetYaxis()->SetTitle("AbsorptionProbabilities");
       AbsorptionProbabilityGraphs->GetYaxis()->SetTitleOffset(1.1);
       AbsorptionProbabilityGraphs->SetMaximum(1.0);
       AbsorptionProbabilityGraphs->SetMinimum(0.001);
     }
     AbsorptionProbabilityGraphs->SetName(Elements[j]->GetChemicalSymbol());
     //AbsorptionProbabilityGraphs->Draw("ap");
     AbsorptionProbabilityGr.push_back(AbsorptionProbabilityGraphs);
     AbsorptionProbEl.clear();
   }
   return AbsorptionProbabilityGr; 
 }
...
void TGasMixture::PlotElectronRange()
{
  ...
  ElectronRangeFunction->GetYaxis()->SetTitleSize(0.02);
  ElectronRangeFunction->GetXaxis()->SetTitleSize(0.02);
