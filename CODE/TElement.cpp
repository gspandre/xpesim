...
{
  MeanIonizationPotential = 0.001*(9.76*AtomicNumber + 58.5/pow(AtomicNumber, 0.19));
}
...
Double_t TElement::GetRutherfordScreeningFactor(Double_t ENERGY)
{
  ...
  return (3.4E-3)*pow(AtomicNumber, 0.67)/ENERGY;
}
...
Double_t TElement::GetRutherfordTotalCrossSection(Double_t ENERGY)
{
  ...
  return (5.21E-21)*(pow(AtomicNumber, 2.0)/pow(ENERGY, 2.0))*
...
Double_t TElement::GetMottTotalCrossSection(Double_t ENERGY)
{
  ...
  Double_t u = log10(8*ENERGY*pow(AtomicNumber, -1.33));
  return (4.7E-18)*(pow(AtomicNumber, 1.33) + 0.032*pow(AtomicNumber, 2.0))/
    ((ENERGY + 0.0155*pow(AtomicNumber, 1.33)*pow(ENERGY, 0.5))*
     (1 - 0.02*pow(AtomicNumber, 0.5)*exp(-u*u)));
}
