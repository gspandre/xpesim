...
TMDP::TMDP()
{
  ...
  ifstream in;
  in.open("WINDOWS/Window_Poly1um_Al006um_Grid018.txt");
...
void   TMDP::FillEfficiencyGraph()
{
  ...
  vector <double> EnVectorG;
  vector <double> EfVectorG;
  for (int i =0;i<N;i++)
    {
  ...
  EfficiencyGraph = new TGraph(N,&EnVectorG[0],&EfVectorG[0]);
}
...
void TMDP::FillMuGraph()
{
  ...
  if (N == 0) std::cout<<" NO DATA in charge "<<std::endl;
  vector <double> EnVectorG;
  vector <double> MuVectorG;
  for (int i =0;i<N;i++)
    {
      EnVectorG[i] = EnergyNVector[i];
      MuVectorG[i] = MuNVector[i];
    }
  Mug= new TGraph(N,&EnVectorG[0],&MuVectorG[0]);
}
...
TGraph *TMDP::FillMDPGraph(TF1 *Source,double Time)
{
  ...
  TGraph *MDPGraph = new TGraph(N,&Energypoint[0],&MDPpoint[0]);
  return MDPGraph;
}
...
void TMDP::CompactPlot()
{
  ...
  */
  ...
  TGraph *MuEp = new TGraph(N,&EnVectorG[0],&MFactor[0]);
  MuEp->SetName("EpMu");
  FindMW(MuEp);
  MuEp->SetLineColor(3);
...
TGraph *TMDP::MuEpGraph()
{
  ...
  TGraph *MuEpGr = new TGraph(N,&EnVectorG[0],&MFactor[0]);
