...
{  
  ...
  //PixMapCanvas = new TCanvas("PixMap","PixMap",800,800);
}
...
//I riga contata dal basso, dipende dalla posizione Y,J colonna contata da sinistra, dipende dalla X, X e Y in cm con origine degli assi al centro del rivelatore
...
{//AGGIUNGO 1 A IMAX E JMAX
  ...
  if ((int)I>=IMin && 
    ...
    {
      std::cout <<"out  "<< " X= "<< Xa<< " Y= "<< Ya <<" I ="<<I<<" J = "<<J<<std::endl;
      // std::pair<int,int> a = make_pair(1000,1000);
      // return  a;
    }
}


//   void TReadout::test(int N=5)
//   {
//   TRandom *rand = new TRandom();
//   TH2D *h1 = new TH2D("h1","h1",80,0,400,80,0,400);
//   TH2D *h2 = new TH2D("h2","h2",80,0,80,80,0,80);
//   std::pair<int,int> co;
//   for(int i = 0;i<N;i++)
//   {
//   double a = rand->Uniform(139,145);
//   double b = rand->Uniform(121,160);
//   co = PositiontoMatrix(a,b);
//   if(i<100)
//   std::cout<<co.first<<" "<<co.second<<std::endl;
//   h1->Fill(a,b);
//   h2->Fill(co.first,co.second);
//   }
...
void TReadout::myDraw2D(int Color, double Size, int LineWidth)
{
  ...
  //  TAxis *yaxis = new TAxis(0.1,0.9,SF*MinX,SF*MaxX);  
  //  view->SetRange(SF*MinX, SF*MinY, 0, SF*MaxX, SF*MaxY,1);
  //  view->Top();
